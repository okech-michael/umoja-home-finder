...
export const Route = createFileRoute("/properties")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Search verified houses for rent in Kisii, Nyamira, Keroka, Ogembo and Suneka. Filter by type, location, price, and amenities." },
    ...
    <div className="pt-28 pb-20 bg-surface min-h-screen">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mb-10 animate-fade-up">
          <span className="text-xs uppercase tracking-widest font-semibold text-primary">Listings</span>
          <h1 className="mt-2 font-display text-4xl md:text-5xl font-bold">Browse Properties</h1>
          <p className="mt-2 text-muted-foreground">Verified homes across Kisii and the wider Gusii Region.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-[300px_1fr] gap-8">
          ...
          <aside className="lg:sticky lg:top-24 self-start space-y-5 rounded-3xl bg-card p-6 shadow-soft h-fit">
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Max Rent (KSh)</label>
              <input
                value={filters.maxRent}
                onChange={(e) => setFilters({ ...filters, maxRent: e.target.value.replace(/[^\d]/g, "") })}
                ...
                {SITE.amenities.map((a) => (
                  <button
                    ...
                    className={`rounded-full px-3 py-1 text-xs font-medium border transition ${
                      filters.amenities.includes(a) ? "gradient-brand text-white border-transparent" : "bg-background hover:bg-muted"
                    }`}
                  >
                    {a}
                  </button>
            ...
            <button
              type="button"
              onClick={() => setFilters({ location: "", propertyType: "", maxRent: "", amenities: [], q: "" })}
          ...
          <div>
            <div className="mb-4 text-sm text-muted-foreground">
              Showing <span className="font-semibold text-foreground">{filtered.length}</span> of {data.length} properties
            </div>
            {filtered.length === 0 ? (
              <div className="rounded-3xl border border-dashed p-16 text-center">
                ...
                <Link to="/contact" className="mt-4 inline-block text-primary font-semibold">Tell us what you're looking for</Link>
              </div>
            ) : (
